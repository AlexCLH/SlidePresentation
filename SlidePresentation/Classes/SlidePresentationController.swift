...
class SlidePresentationController: UIPresentationController {
    ...
    override var frameOfPresentedViewInContainerView: CGRect {
        ...
        if presentedSize == .zero {
            presentedSize = size(forChildContentContainer: presentedViewController, withParentContainerSize: containerView?.bounds.size ?? windowSize)
        }
        ...
        switch style {
        case .alert(_):
            frame.origin = CGPoint(x: (width - presentedSize.width) / 2, y: (height - presentedSize.height) / 2)
        case .actionSheet(.right):
            frame.origin.x = width - presentedSize.width
            frame.size.height = height
        case .actionSheet(.bottom):
            frame.origin.y = height - presentedSize.height
...
private extension SlidePresentationController {
    ...
    @objc func keyBoardWillShow(notification: Notification) {
        ...
        if let keyBoardFrame = notification.keyboardEndFrame() {
            ...
            keyBoardAnimationDuration = notification.keyboardAnimationDuration() ?? 0.25
    ...
    func transitionFrame(from keyBoardFrame: CGRect) -> CGRect {
        ...
        case .alert(_):
            if pBottom > kY {
                presentedFrame.origin.y = UIScreen.main.bounds.height - keyBoardFrame.height - presentedFrame.height + keyBoardHeightOffset
            }
        case .actionSheet(.bottom):
...
extension SlidePresentationController: UIGestureRecognizerDelegate {
    ...
    func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer, shouldRecognizeSimultaneouslyWith otherGestureRecognizer: UIGestureRecognizer) -> Bool {
        if gestureRecognizer == pan,
            (otherGestureRecognizer.isKind(of: (NSClassFromString("UIScrollViewPanGestureRecognizer") ?? UIView.self)) == true || otherGestureRecognizer is UIPanGestureRecognizer ),
...
extension SlidePresentationController {
    ...
    private func bottomPanClose(with translation: CGPoint) {
        ...
        if isDragScrollView {
            ...
            let oy = UIScreen.main.bounds.height - contentFrame.height
            if translation.y > 0 {
                contentFrame.origin.y += translation.y
                contentView?.frame = contentFrame
    ...
    private func rightPanClose(with translation: CGPoint) {
        var contentFrame = contentView?.frame ?? .zero
        let ox = UIScreen.main.bounds.width - contentFrame.width
        if translation.x > 0 {
            contentFrame.origin.x += translation.x
            contentView?.frame = contentFrame
